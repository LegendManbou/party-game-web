...
- トップ画面に「その他ゲーム」機能を追加し、その他ゲーム一覧を開けます。
...
`game` はワードウルフ用、`spyHunt` と `spyGuesses` はスパイ探し用です。既存のワードウルフ部屋に `gameType` が無い場合は、ワードウルフとして扱います。
...
2. トップ画面に「ワードウルフ」と「スパイ探し」のカードが表示されることを確認します。
...
7. 退出、ホスト退出後のホスト変更、スマホ幅表示を確認します。
